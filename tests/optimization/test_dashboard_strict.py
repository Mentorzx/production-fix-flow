...
DASHBOARD_HTML = (
    settings.ROOT_DIR / "pff/infrastructure/hpo/dashboard/static/index.html"
)


@pytest.mark.xfail(
    reason="Current dashboard prototype uses Tailwind Play CDN (Technical Debt)"
)
...
@pytest.mark.xfail(
    reason="Current dashboard prototype uses Babel Standalone (Technical Debt)"
)
...
def test_no_adjacent_jsx_in_inline_svg():
    ...
    # Let's look for known bad patterns: d={<path .../><path .../>}
    # We want to ensure if multiple tags are present, they are wrapped.

    # Simple check: find all d={...} and inspect
    matches = re.finditer(r"d=\{([^}]+)\}", html)

    for m in matches:
        content = m.group(1)
        # Count root-level tags in this snippet
        # If it starts with <>, it is wrapped (Fragment)
        if content.strip().startswith("<>"):
            continue

        # If it doesn't start with <>, it must be a single element
        # Count occurences of "/>" or "</"
