...
class TestMatchParamsFloatPrecision:
    ...
    def test_nan_float_comparison(self, manager: MockTrialArtifactManager):
        ...
        # abs(nan - nan) > 1e-9 is always False because nan comparisons are False
        # Actually: abs(nan - nan) is nan, and nan > 1e-9 is False
        result = manager._match_params(stored, trial_params)
        # This is counterintuitive: NaN "matches" NaN due to IEEE behavior
        assert result is True  # Bug or feature?
