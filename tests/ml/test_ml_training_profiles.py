...
class TestMLTrainingProfileGeneration:
    ...
    def test_low_spec_profile_safe_limits(self):
        ...
        assert profile.rotate.embedding_dim == 64  # Reduced dimension
        assert profile.rotate.batch_size <= 512  # Small batch
        assert profile.rotate.max_entities == 50_000  # Hard limit
        assert profile.rotate.use_gpu is False
    ...
    def test_mid_spec_profile_moderate_config(self):
        ...
        assert profile.rotate.embedding_dim == 128  # Full dimension
        assert profile.rotate.batch_size == 512
        assert profile.rotate.max_entities == 200_000  # Limited but reasonable
        assert profile.rotate.num_epochs == 50  # Reduced for faster iteration
    ...
    def test_high_spec_profile_full_training(self):
        ...
        assert profile.rotate.embedding_dim == 256  # Large embeddings
        assert profile.rotate.batch_size == 2048  # Large batch for GPU
        assert profile.rotate.max_entities is None  # No limit
        assert profile.rotate.use_gpu is True
        assert profile.rotate.num_epochs == 100  # Full training
        assert profile.anyburl.max_memory_gb == 20
        assert profile.ray_object_store_memory_gb == 8

    def test_gpu_detection_affects_rotate_config(self):
        ...
        assert profile_no_gpu.rotate.use_gpu is False
        ...
        assert profile_with_gpu.rotate.use_gpu is True
...
class TestMLTrainingProfileOOMPrevention:
    ...
    def test_entity_limit_prevents_oom_low_spec(self):
        ...
        assert profile.rotate.max_entities is not None
        assert profile.rotate.max_entities == 50_000
    ...
    def test_entity_limit_increased_for_mid_spec(self):
        ...
        assert profile.rotate.max_entities is not None
        assert profile.rotate.max_entities == 200_000
    ...
    def test_no_entity_limit_for_high_spec(self):
        ...
        assert profile.rotate.max_entities is None
...
class TestMLTrainingProfileIntegration:
    ...
    def test_get_ml_training_profile_returns_valid_config(self):
        ...
        assert profile.rotate is not None
    ...
    def test_thread_count_respects_hardware_limits(self):
        ...
        assert profile.rotate.num_workers <= hardware.cpu_threads
