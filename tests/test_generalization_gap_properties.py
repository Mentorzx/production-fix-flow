...
class TestGeneralizationGapComputation:
    ...
    @pytest.mark.parametrize("val_auc,test_auc,expected_gap", [
...
class TestScoreWithGapPenalty:
    ...
    def test_coefficient_scales_penalty_impact(self, gap_coeff: float):
        ...
        score_no_penalty = base_score  # No penalty applied
...
class TestGapPenaltyConfiguration:
    ...
    def test_custom_threshold_respected(self, threshold: float):
        ...
        assert penalty_at == 0.0, f"At threshold should give 0 penalty"
        assert penalty_above > 0.0, f"Above threshold should give positive penalty"
