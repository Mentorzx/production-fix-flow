...
class TestBloomFilter:
    ...
    def test_bloom_filter_missing_item(self) -> None:
        ...
        bf = BloomFilter(expected_items=1000, false_positive_rate=0.01)
        bf.add(42)
        # Note: False positives are possible but rare at 1%
        # Testing with many items increases chance of false positive
        # Here we just test basic behavior
        missing_detected = sum(1 for i in range(1000, 2000) if bf.might_contain(i))
        # Should be around 1% false positive rate
        assert missing_detected < 50  # Allow some margin
...
class TestCalculateOptimalBatchSize:
    ...
    def test_batch_size_scales_with_dtype(self) -> None:
        ...
        float32 = calculate_optimal_batch_size(
            n_features=1100, dtype=np.dtype(np.float32)
        )
        float64 = calculate_optimal_batch_size(
            n_features=1100, dtype=np.dtype(np.float64)
        )
...
@pytest.mark.skipif(
    not NUMBA_AVAILABLE, reason="Numba required for binary_search_range"
)
