...
def _build_model(lambda_pc: float) -> DSLFMKGCModel:
    ...
    model._all_entity_features = torch.zeros(
        config.num_entities, config.feature_dim
    )  # noqa: SLF001
    model._all_entity_communities = torch.zeros(
        config.num_entities, config.max_communities
    )  # noqa: SLF001
    return model
...
def test_pc_rerank_changes_winner_when_lambda_positive(monkeypatch) -> None:
    ...
    pc_log_full = torch.tensor([[0.0, 0.0, 4.0]])  # Boost candidate 2
    tails = torch.tensor([2])  # True tail is index 2
    ...
    monkeypatch.setattr(
        model0, "_pc_log_prob_matrix", lambda *_, **__: pc_log_full[:, :3]
    )  # not used when lambda=0
    ...
    assert metrics0["mrr"] < 0.6  # tail is not top-2 by decoder

    # Lambda>0 → PC should push candidate 2 to the top via rerank
    model1 = _build_model(lambda_pc=1.5)
    monkeypatch.setattr(
        model1.decoder, "score_all_tails", lambda **_: decoder_scores.clone()
    )
    ...
    monkeypatch.setattr(
        model1, "_pc_log_prob_matrix", lambda *_, **__: pc_log_full.clone()
    )
    # Also patch pairwise for the true score calculation
    monkeypatch.setattr(
        model1, "_pc_log_prob_pairwise", lambda *_, **__: torch.tensor([4.0])
    )
    ...
    assert metrics1["mrr"] == 1.0  # tail promoted to rank 1
...
def test_pc_rerank_respects_topk_mask(monkeypatch) -> None:
    ...
    pc_log = torch.tensor([[2.0, 0.0, -10.0]])  # Match shape (1, 3)
    ...
    monkeypatch.setattr(
        model, "_pc_log_prob_pairwise", lambda *_, **__: torch.tensor([2.0])
    )
...
def test_pc_can_dominate_with_high_lambda(monkeypatch) -> None:
    ...
    pc_log = torch.tensor([[0.0, 0.0, 5.0]])  # PC favorece candidato 2
    ...
    monkeypatch.setattr(model, "_pc_log_prob_matrix", lambda *_, **__: pc_log.clone())
    monkeypatch.setattr(
        model.decoder, "forward", lambda **_: torch.tensor([1.0])
    )  # Tail 2 score
    monkeypatch.setattr(
        model, "_pc_log_prob_pairwise", lambda *_, **__: torch.tensor([5.0])
    )  # Tail 2 PC score
...
def test_rerank_topk_applies_inf_outside_batch(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        model, "_pc_log_prob_pairwise", lambda *_, **__: torch.tensor([0.5, 0.0])
    )
