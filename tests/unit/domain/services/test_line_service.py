...
class TestLineServicePerformance:
    ...
    async def test_circuit_breaker_fails_fast(self):
        ...
        with patch.object(service, "make_request", side_effect=failing_request):
            ...
            # Check for circuit breaker error message
            error_msg = result4["error"].lower()
            details_msg = result4.get("details", "").lower()
            assert (
                "temporarily unavailable" in error_msg
                or "indisponivel" in error_msg
                or "unavailable" in details_msg
                or "indisponivel" in details_msg
            )
