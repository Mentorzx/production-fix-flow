...
NOTE: These tests require the full schema with users, telecom_data, kg_embeddings,
      kg_triples, and execution_logs tables. Skip if only KG tables exist.
"""
...
# Skip all tests in this module if DATABASE_URL not configured
pytestmark = [
...
async def db_connection():
    ...
    # Check if legacy schema tables exist
    ...
    if not tables:
        await conn.close()
        pytest.skip("Legacy schema tables (users, telecom_data) not found - run migrations first")
