...
These tests validate the Learn command with different model types (kg, rotate, ensemble, all).
...
@pytest.mark.skip(reason="LearnCommand refactoring in progress - Sprint 18")
class TestLearnCommandRotatE:
    """Tests for RotatE-only learning."""

    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_learn_rotate_only(self):
        """Test learning RotatE embeddings only."""
        args = argparse.Namespace(model="rotate", config=None)
        cmd = LearnCommand(args)
        
        with patch("pff.validators.rotate.manager.RotatEManager") as mock_manager:
            mock_instance = Mock()
...
class TestLearnCommandAll:
    ...
    async def test_learn_all_models(self):
        ...
        with patch("pff.validators.kg.pipeline.KGPipeline") as mock_kg, \
             patch("pff.validators.rotate.manager.RotatEManager") as mock_rotate, \
            ...
            mock_rotate_instance = Mock()
            mock_rotate_instance.train = AsyncMock()
            mock_rotate.return_value = mock_rotate_instance
            ...
            mock_rotate_instance.train.assert_called_once()
...
def test_learn_command_with_custom_config():
    ...
    args = argparse.Namespace(model="rotate", config=str(custom_config))
    cmd = LearnCommand(args)
    
    assert cmd.model == "rotate"
