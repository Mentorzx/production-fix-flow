...
def test_filter_mask_removes_known_tails() -> None:
    ...
    # Both _filter_dict AND _filter_entries_cpu are required for _mask_known_tails
    manager._filter_dict = {(0, 0): {1, 2}}
    # Entries: (head, relation, tail) for each known triple
    manager._filter_entries_cpu = torch.tensor([[0, 0, 1], [0, 0, 2]], dtype=torch.long)
