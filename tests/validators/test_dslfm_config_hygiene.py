...
def test_legacy_config_also_cleaned() -> None:
    ...
    from pff.domain.learning.dslfm.config import DSLFMConfig

    config = DSLFMConfig()

    # Verify cleanup was applied
    assert not hasattr(config, "gamma"), "gamma was removed from legacy DSLFMConfig"
    assert not hasattr(config, "epsilon"), "epsilon was removed from legacy DSLFMConfig"
