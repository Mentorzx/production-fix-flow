...
def test_pc_rerank_changes_winner_when_lambda_positive(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        model0, "_pc_log_prob_subset", lambda **_: pc_log_full[:, :3]
    ...
    monkeypatch.setattr(model1, "_pc_log_prob_subset", lambda **_: pc_log_full.clone())
...
def test_pc_rerank_respects_topk_mask(monkeypatch) -> None:
    ...
    pc_log = torch.tensor([[2.0, 0.0]])  # Only for top-2
    ...
    monkeypatch.setattr(model, "_pc_log_prob_subset", lambda **_: pc_log.clone())
    ...
    updated = torch.log_softmax(topk.values, dim=1) + model.config.lambda_pc * pc_log
...
def test_rerank_topk_applies_inf_outside_batch(monkeypatch) -> None:
    """Em batch, candidatos fora do top-k devem ficar -inf em todas as linhas."""
    decoder_scores = torch.tensor([[4.0, 1.0, 0.5], [0.1, 3.0, 0.2]])
    pc_log = torch.tensor([[0.0, 0.5], [0.0, -0.5]])  # k=2
