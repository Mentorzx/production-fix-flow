...
def test_symbolic_features_are_deterministic(sample_rules, sample_data):
    ...
    print(
        f" Symbolic features are deterministic across 3 runs for {len(sample_data)} samples"
    )
...
def test_sparsity_variance_is_below_threshold(sample_rules, sample_data):
    ...
    for run in range(3):
        ...
        sparsity = (
            (total_violations / total_elements) * 100 if total_elements > 0 else 0
        )
    ...
    print(
        f"Mean: {mean_sparsity:.4f}%, Variance: {variance:.4f}% ({variance_pct:.2f}%)"
    )
...
def test_determinism_with_numba_parallel(sample_rules):
    ...
    print(
        f" Determinism maintained with Numba parallel processing ({len(large_samples)} samples)"
    )
