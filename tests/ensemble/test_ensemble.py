...
- RotatEWrapper (KG embeddings wrapper)
- HybridWrapper (RotatE + LightGBM)
...
from pff.validators.ensembles.ensemble_wrappers.model_wrappers import RotatEWrapper, HybridWrapper
...
class TestRotatEWrapper:
    """Test RotatEWrapper functionality."""
    ...
    @pytest.fixture
    def mock_rotate_config_path(self, tmp_path):
        """Create a mock RotatE config path."""
        config_path = tmp_path / "rotate_config.json"
        config_path.write_text('{"model": {"embedding_dim": 128, "gamma": 9.5}}')
        return str(config_path)

    def test_rotate_wrapper_initialization(self, mock_kg_config_path, mock_rotate_config_path):
        """Test RotatEWrapper initializes correctly."""
        wrapper = RotatEWrapper(
            kg_config_path=mock_kg_config_path,
            rotate_config_path=mock_rotate_config_path
        )

        assert wrapper.kg_config_path == mock_kg_config_path
        assert wrapper.rotate_config_path == mock_rotate_config_path
        assert wrapper.scorer_service_ is None  # Not initialized until fit()
        assert wrapper.timeout == 30.0

    def test_rotate_wrapper_inherits_base_wrapper(self, mock_kg_config_path, mock_rotate_config_path):
        """Test RotatEWrapper inherits from BaseWrapper."""
        wrapper = RotatEWrapper(
            kg_config_path=mock_kg_config_path,
            rotate_config_path=mock_rotate_config_path
        )
    ...
    def test_rotate_wrapper_cache_key_generation(self, mock_kg_config_path, mock_rotate_config_path):
        ...
        # Cache key should contain config file stems with rotate prefix
        assert "kg_config" in wrapper._cache_key
        assert "rotate_config" in wrapper._cache_key
        assert wrapper._cache_key.startswith("rotate_")

    def test_rotate_wrapper_getstate_removes_unpicklable(self, mock_kg_config_path, mock_rotate_config_path):
    ...
    def test_rotate_wrapper_setstate_restores_managers(self, mock_kg_config_path, mock_rotate_config_path):
    ...
    @patch('pff.validators.ensembles.ensemble_wrappers.model_wrappers.RotatEScorerService')
    @patch('pff.validators.ensembles.ensemble_wrappers.model_wrappers.KGConfig')
    def test_rotate_wrapper_fit_initializes_scorer(
        self, mock_kg_config, mock_scorer_service, mock_kg_config_path, mock_rotate_config_path
    ):
        ...
        # Mock the KGConfig and RotatEScorerService
    ...
    def test_rotate_wrapper_predict_proba_shape(self, mock_kg_config_path, mock_rotate_config_path):
    ...
    def test_rotate_wrapper_predict_uses_threshold(self, mock_kg_config_path, mock_rotate_config_path):
...
class TestHybridWrapper:
    """Test HybridWrapper (RotatE + LightGBM hybrid model)."""
...
class TestSklearnCompatibility:
    ...
    def test_wrapper_has_classes_attribute(self, tmp_path):
        ...
        rotate_path = tmp_path / "rotate.json"
        kg_path.write_text('{}')
        rotate_path.write_text('{}')

        wrapper = RotatEWrapper(str(kg_path), str(rotate_path))
    ...
    def test_wrapper_get_set_params(self, tmp_path):
        ...
        assert 'rotate_config_path' in params

        # set_params should work
        new_rotate_path = tmp_path / "new_rotate.json"
        new_rotate_path.write_text('{}')
        wrapper.set_params(rotate_config_path=str(new_rotate_path))
        assert wrapper.rotate_config_path == str(new_rotate_path)
...
class TestWrapperSerialization:
    ...
    def test_wrapper_unpickle_restores_managers(self, tmp_path):
        ...
        state = wrapper.__getstate__()
        new_wrapper = RotatEWrapper.__new__(RotatEWrapper)
    ...
    def test_wrapper_state_preserves_config_paths(self, tmp_path):
        ...
        assert state['rotate_config_path'] == str(rotate_path)
