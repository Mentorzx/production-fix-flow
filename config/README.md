...
- `hpo/`
  ...
  - `optimization.yaml` – HPO memory, storage (Optuna), sampler/pruner, bounds, and live dashboard toggles
    - `defaults.inverse_relation_policy`: `drop_suffix|keep` para controlar remocao de `*_inv` no carregamento HPO
    - `defaults.inverse_suffix`: sufixo da relacao inversa (padrao `_inv`)
    - `defaults.relation_id_policy`: `preserve_sparse|auto|remap_dense` para IDs de relacao nao contiguos
- `infra/`
