...
def clear_config_cache() -> None:
    ...
    cache = _get_config_cache()
    cache.invalidate(tags=["hpo_config"])
...
def _read_hpo_config(file_manager: FileManager | None = None) -> dict[str, Any]:
    """Read HPO config strictly from optimization.yaml."""
    fm = file_manager or FileManager()
    return load_optimization_config(DEFAULT_OPT_PATH, fm)
...
def load_parallel_settings(file_manager: FileManager | None = None) -> dict[str, Any]:
    ...
    Returns:
        ...
        - optuna.gc_after_trial: Trigger gc collection after each Optuna trial.
        - cv.*: Policy switches for enabling/disabling CV parallelism guards.
    """
    cfg = _read_hpo_config(file_manager)
    parallel = cfg.get("parallel", {})
    if not isinstance(parallel, dict):
        parallel = {}
    optuna = parallel.get("optuna", {})
    ...
    return {
        ...
        "optuna": {
            "gc_after_trial": bool(optuna.get("gc_after_trial", True)),
        },
        "cv": {
            "disable_when_cuda": bool(cv.get("disable_when_cuda", True)),
            "disable_when_dataloader_workers": bool(
                cv.get("disable_when_dataloader_workers", True)
            ),
            "disable_when_auto_workers": bool(
                cv.get("disable_when_auto_workers", True)
            ),
        },
    }
...
def load_hpo_defaults(file_manager: FileManager | None = None) -> dict[str, Any]:
    ...
    dslfm_kgc = cfg.get("dslfm_kgc", {})
    if not isinstance(dslfm_kgc, dict):
        return {}
    if not dslfm_kgc:
        defaults: dict[str, Any] = cfg.get("dslfm_defaults", {})
        return defaults

    flat: dict[str, Any] = {}
    training_raw = dslfm_kgc.get("training", {})
    ...
    arch_raw = dslfm_kgc.get("architecture", {})
    arch = arch_raw if isinstance(arch_raw, dict) else {}
    ...
    contr_raw = dslfm_kgc.get("contrastive", {})
    sampling_raw = dslfm_kgc.get("sampling", {})
    contr = contr_raw if isinstance(contr_raw, dict) else {}
    ...
    pc_raw = dslfm_kgc.get("pc", {})
    pc = pc_raw if isinstance(pc_raw, dict) else {}

    def _require_pc_value(key: str) -> Any:
        if key not in pc:
            raise ValueError(
                f"Missing required config value: dslfm_kgc.pc.{key} in optimization.yaml"
            )
        return pc[key]

    flat["lambda_pc_low"] = pc.get("lambda_pc_low", 0.1)
    flat["lambda_pc_high"] = pc.get("lambda_pc_high", 2.0)
    flat["pruning_threshold_low"] = float(_require_pc_value("pruning_threshold_low"))
    flat["pruning_threshold_high"] = float(_require_pc_value("pruning_threshold_high"))
    flat["rebuild_every_low"] = int(_require_pc_value("rebuild_every_low"))
    flat["rebuild_every_high"] = int(_require_pc_value("rebuild_every_high"))
    flat["max_circuit_depth_choices"] = pc.get("depth_choices", [2, 3, 4])

    defaults = dslfm_kgc.get("defaults", {})
...
def load_storage_settings(file_manager: FileManager | None = None) -> dict[str, Any]:
    ...
    if not isinstance(grpc_cfg, dict):
        grpc_cfg = {}
    env_storage_backend = os.getenv("PFF_HPO_STORAGE_BACKEND")
    env_storage_url = os.getenv("PFF_HPO_STORAGE_URL")
    ...
    return {
        "backend": str(
            env_storage_backend
            if env_storage_backend is not None
            else storage_cfg.get("backend", defaults["backend"])
        ),
        "url": (
            env_storage_url
            if env_storage_url is not None
            else storage_cfg.get("url", defaults["url"])
        ),
        "engine": {
            "pool_size": int(
...
def load_live_plot_settings(file_manager: FileManager | None = None) -> dict[str, Any]:
    """Execute load live plot settings.
...
def load_trial_constraints(file_manager: FileManager | None = None) -> dict[str, Any]:
    """Execute load trial constraints.
