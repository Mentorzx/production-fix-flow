...
export const MetricCard = ({
  ...
  const themeColors = {
    ...
    orange: {
      main: Theme.palette.hotOrange,
      bg: "rgba(249,115,22,0.15)",
      border: "rgba(249,115,22,0.3)",
    },
    lime: {
      main: Theme.palette.vividGreen,
      bg: "rgba(132,204,22,0.15)",
      border: "rgba(132,204,22,0.3)",
    },
    amber: {
      main: Theme.palette.cyberYellow,
...
export const PaceCard = ({ trials }) => {
  const { value, subtext } = useMemo(() => {
    const completed = trials.filter(
      (t) => t.state === "COMPLETE" && typeof t.duration === "number" && t.duration > 0.1
    );
...
export const CompletionCard = ({ trials, totalTrials = DEFAULT_TOTAL_TRIALS }) => {
  ...
  return <MetricCard label="Conclusão Estimada" value={value} subtext={subtext} color="amber" />;
};
