...
const LEVEL_STYLE = {
  ...
  WARNING: {
    ...
    bgHover: "rgba(202, 138, 4, 0.20)",
    border: "rgba(202, 138, 4, 0.25)",
    text: "#fde68a",
    badge: "#eab308",
    badgeBg: "rgba(202, 138, 4, 0.20)",
  },
  CRITICAL: {
    bg: "rgba(220, 38, 38, 0.18)",
    bgHover: "rgba(220, 38, 38, 0.30)",
...
const HEADER_HINTS = {
  ...
  level: {
    ...
    extra: [
      { label: "WARN", value: "Atenção necessária" },
      { label: "ERROR", value: "Falha no fluxo" },
      { label: "CRITICAL", value: "Sistema em risco" },
    ],
  },
  module: {
...
const LogHeaderCell = ({ label, hint }) => {
  const TooltipContent = (
    <div
      className="w-56 border p-2.5 rounded-xl shadow-2xl text-left normal-case font-sans z-50"
  ...
  return (
    <PortalTooltip content={TooltipContent}>
      <span className="cursor-help border-b border-dotted border-zinc-700 hover:border-zinc-500 transition-colors">
        {label}
      </span>
    </PortalTooltip>
  );
...
export const TerminalLogCard = ({ logs }) => {
  ...
  const totalPages =
    rowsPerPage === "All" ? 1 : Math.max(1, Math.ceil(entries.length / rowsPerPage));
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        className="w-full flex flex-col font-mono text-[10px] rounded-lg border shadow-inner overflow-hidden"
