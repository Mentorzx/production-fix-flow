...
export const BestTrialCard = ({ trial }) => {
    ...
    useEffect(() => {
        let start = 0; const end = trial?.id || 0; if (start === end) return;
        let timer = null; const duration = 2000; const stepTime = 20; const steps = duration / stepTime;
        const increment = (end - start) / steps; let current = start;
        timer = setInterval(() => {
            current += increment;
            if ((increment > 0 && current >= end) || (increment < 0 && current <= end)) { setDisplayId(end); clearInterval(timer); }
            else { setDisplayId(Math.floor(current)); }
        }, stepTime);
            ...
            <div className="flex flex-col h-full">
                ...
                <div className="space-y-4 flex-1 overflow-y-auto max-h-[400px] custom-scrollbar pff-stagger">
                    {Object.entries(trial.params).map(([key, val], idx) => (
                        <div key={key} className="flex justify-between items-center border-b border-zinc-800/60 pb-2" style={{ "--stagger": idx * 35 }}>
                            {renderParamWithHints(key.replace(/_/g, ' '), typeof val === 'number' ? val.toFixed(4) : String(val))}
                            <span className="font-mono tabular-nums font-bold text-sm" style={{ color: 'var(--viz-text-primary)' }}>{typeof val === 'number' ? val.toFixed(4) : String(val)}</span>
                        </div>
                    ))}
                </div>
            </div>
