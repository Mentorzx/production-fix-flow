...
export const ELBOBreakdownCard = ({ liveStatus }) => {
    ...
    return (
        <Card title="ELBO Breakdown" icon={BarChart2} className="h-full" helpText={ChartRegistry.get('elbo_breakdown')}>
            {hasData ? (
                    ...
                    <div className="flex items-center justify-between border-b border-zinc-800/50 pb-2 flex-none">
                        ...
                        <div className="flex flex-col">
                            <div className="flex items-baseline gap-2">
                                <span className="text-xl font-mono text-rose-400 leading-none">{latest.recon.toFixed(4)}</span>
                                <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">Recon</span>
                            </div>
                            <span className="text-[10px] text-zinc-600">{(latest.recon / latest.total * 100).toFixed(1)}% weight</span>
                        </div>
                        ...
                        <div className="flex flex-col items-end">
                            <div className="flex items-baseline gap-2">
                                <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">KL Div</span>
                                <span className="text-xl font-mono text-orange-400 leading-none">{latest.kl.toFixed(4)}</span>
                            </div>
                            <span className="text-[10px] text-zinc-600">{(latest.kl / latest.total * 100).toFixed(1)}% weight</span>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <ChartContainer minHeight={120} className="h-full">
                            <AreaChart data={data} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
                                ...
                                <Legend formatter={renderWithHints} verticalAlign="top" align="left" height={18} wrapperStyle={{ top: -8, fontSize: '10px' }} />
                                <Area type="monotone" dataKey="recon" stackId="1" stroke={Theme.semantic.chart.recon} fill="url(#gradRecon)" strokeWidth={2} name="Recon" />
                                <Area type="monotone" dataKey="kl" stackId="1" stroke={Theme.semantic.chart.klDiv} fill="url(#gradKL)" strokeWidth={2} name="KL Div" />
                            </AreaChart>
                        </ChartContainer>
                    </div>
                </div>
