...
import {
  ...
  formatParamValue,
  resolveMetricValue,
} from "../../../domain/metrics/Formatters.js";
              ...
              {selected.map((trial, idx) => (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span
                      className={`text-[9px] uppercase tracking-widest ${idx === 0 ? "text-lime-400" : idx === 1 ? "text-amber-400" : "text-rose-400"}`}
                    >
                      {idx === 0 ? "melhor" : idx === 1 ? "recente" : "pior"}
                    </span>
                    <span className="text-[9px] text-zinc-500">
                      Score: {formatMetricValue(resolveMetricValue(trial, "score"))}
                    </span>
                  </div>
                </th>
              ))}
                ...
                <tr key={row.key} className="dashboard-table-row border-b border-zinc-800/50">
                  <td className="p-2 text-orange-400">
                    {row.type === "metric"
                      ...
                      : renderParamWithHints(row.label)}
                  </td>
                  {values.map((value, idx) => {
                    ...
                    return (
                      <td
                        key={`${row.key}-${idx}`}
                        className={`p-2 ${isBest ? "text-lime-300" : "text-zinc-300"}`}
                      >
                        {formatted}
                      </td>
                    );
