...
export const Dashboard = () => {
  const { data, activeTab, setActiveTab, viewMode, setViewMode, isRunning, currentTime } =
    useStore();
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col z-10 pointer-events-none">
        ...
        <main
          className={`flex-1 overflow-auto custom-scrollbar p-6 transition-opacity duration-300 pointer-events-auto ${isPending ? "opacity-50" : "opacity-100"}`}
        >
        ...
        <footer
          className="flex-none h-8 px-4 flex items-center justify-between text-[10px] font-mono pointer-events-auto"
          style={{
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <span className="border-l border-zinc-800 pl-4">
              Dashboard: v{versionInfo.version} (build {versionInfo.buildId})
            </span>
          </div>
          <div>
            Last Update:{" "}
            {data.updatedAt
              ? new Date(data.updatedAt).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" })
              : "N/A"}
          </div>
        </footer>
      </div>
