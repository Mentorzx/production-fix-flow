...
export const EmptyState = React.memo(({ children, className = "" }) => (
  <div
    className={`h-full flex items-center justify-center italic text-xs ${className}`}
...
export const BaseTooltip = React.memo(({ trigger, title, tech, className = "inline-block" }) => {
    ...
    >
      {title && (
        <div
          ...
          style={{ borderColor: Theme.ui.border, color: Theme.ui.text.primary }}
        >
          {title}
        </div>
      )}
  ...
  return (
    <div className={`${className} h-full w-full`}>
      <PortalTooltip content={tooltipContent} className="h-full w-full block">
        {trigger}
      </PortalTooltip>
    </div>
  );
...
export const EmptyStatePulso = React.memo(
  ({
    ...
    return (
      <div
        className={`h-full flex flex-col items-center justify-center p-8 text-center ${className}`}
      >
        ...
        <h3 className="text-sm font-semibold mb-2" style={{ color: config.textColor }}>
          {title}
        </h3>
        {subtitle && (
          <p
...
export const WithDataPulso = React.memo(
  ({
    ...
    when ? (
      ...
      <EmptyStatePulso title={title} subtitle={subtitle} mood={mood} className={className} />
    )
);
