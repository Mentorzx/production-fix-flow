  ...
  return (
    <div className="relative">
      <button
        ...
        className="bg-zinc-900 border border-zinc-700 p-1.5 rounded-lg text-zinc-300 hover:bg-zinc-800"
      >
        <Download size={14} />
      </button>
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-32 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-50 overflow-hidden">
          {["json", "csv", "parquet", "toon"].map((fmt) => (
            <button
              ...
              className="w-full text-left px-4 py-2 text-xs text-zinc-300 hover:bg-orange-500/10 hover:text-orange-300 uppercase font-bold"
            >
              {fmt}
            </button>
          ))}
