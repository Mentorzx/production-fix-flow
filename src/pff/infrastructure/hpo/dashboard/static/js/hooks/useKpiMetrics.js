...
export const useKpiMetrics = () => {
  ...
  const bestSeries = useMemo(() => {
    ...
    return items
      .filter(
        (t) =>
          t && t.state === "COMPLETE" && typeof t.value === "number" && Number.isFinite(t.value)
      )
      .sort((a, b) => a.id - b.id)
      .map((t) => {
        const val = t.value;
  ...
  const avgDurationDeltaPct = useMemo(
    () => calcDeltaPct(avgDuration, avgDurationPrev),
  ...
  const etaParts = useMemo(
    () => formatCompactDurationParts(estimatedCompletion.eta),
    [estimatedCompletion.eta]
  );

  const liveTrialHistory = useMemo(() => {
    const rows = Array.isArray(data?.liveStatus?.epoch_history)
  ...
  const liveMetricSeries = useMemo(() => {
    ...
    for (const e of liveTrialHistory) {
      ...
      const lossVal = parseNumber(
        payload.loss ?? payload.train_loss ?? payload.val_loss ?? payload.binary_loss
      );
