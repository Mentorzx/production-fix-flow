...
def update_adaptive_performance_state(
    *,
    ...
    is_degraded = float(compute_ms) >= float(threshold_ms) and float(validation_lb) >= float(
        validation_lb_min
    )
...
def resolve_runtime_settings(
    *,
    ...
    configured_distribution_conflicts = (
        list(cfg_distribution_conflicts) if isinstance(cfg_distribution_conflicts, list) else []
    )

    explicit_surrogate = isinstance(advisor_config, dict) and "enable_surrogate" in advisor_config
    ...
    explicit_internal_importances = (
        isinstance(advisor_config, dict) and "disable_internal_importances" in advisor_config
    )
    ...
    return RuntimeSettings(
        ...
        effective_min_trials_any=int(effective_cfg.get("min_trials_any", min_trials_any_default)),
        effective_min_trials_aggressive=int(
            effective_cfg.get("min_trials_aggressive", min_trials_aggressive_default)
        ),
        conditional_scope_threshold=float(effective_cfg.get("conditional_scope_threshold", 0.9)),
        ...
        adaptive_perf_ms_threshold=float(effective_cfg.get("adaptive_perf_ms_threshold", 1200.0)),
        adaptive_perf_validation_lb_min=float(
            effective_cfg.get("adaptive_perf_validation_lb_min", 0.75)
        ),
        adaptive_perf_cooldown_calls=int(effective_cfg.get("adaptive_perf_cooldown_calls", 2)),
        rust_spearman_min_len=int(
            effective_cfg.get("rust_spearman_min_len", rust_spearman_min_len_default)
        ),
        self_audit_period_trials=int(
            effective_cfg.get("self_audit_period_trials", self_audit_period_trials_default)
        ),
        self_audit_min_prefix=int(
        ...
        self_audit_max_prefixes=int(
            effective_cfg.get("self_audit_max_prefixes", self_audit_max_prefixes_default)
        ),
        self_audit_min_group_total=int(
            effective_cfg.get("self_audit_min_group_total", self_audit_min_group_total_default)
        ),
        self_audit_wilson_block=float(
            effective_cfg.get("self_audit_wilson_block", self_audit_wilson_block_default)
        ),
        self_audit_min_match_points=int(
            effective_cfg.get("self_audit_min_match_points", self_audit_min_match_points_default)
        ),
        categorical_min_topk_samples=int(
    ...
    *,
    ...
    if adaptive_perf_enabled:
        ...
        if should_degrade_controls:
            ...
            if degraded_count_before >= 2 and resolved_enable_surrogate and not explicit_surrogate:
