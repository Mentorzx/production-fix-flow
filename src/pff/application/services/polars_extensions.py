    ...
    def load_dataframe(self, key: str) -> pl.DataFrame | None:
        ...
        try:
            ...
            if isinstance(bundle, ParquetBundle):
                df = bundle.lazyframe().collect(engine="streaming")
            elif isinstance(bundle, pl.LazyFrame):
                df = bundle.collect(engine="streaming")
            else:
                df = bundle
...
def json_response_to_parquet(
    ...
    if df is not None:
        ...
        compression_typed = cast(
            ParquetCompression,
            compression if compression in valid_compressions else "lz4",
        )
        df.write_parquet(output_path, compression=compression_typed)  # type: ignore[arg-type]
        return output_path
