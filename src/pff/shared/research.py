...
class _TripleIndexStrategy(SearchStrategy):
    ...
    def __init__(self) -> None:
        ...
        self.triples_cache = DiskCache(root=settings.CACHE_DIR / "triples_cache")
    ...
    def _generate_cache_key(self, data: Any) -> str:
        ...
        if isinstance(data, list) and all(isinstance(item, dict) for item in data):
            ...
            return f"{stable_hash(combined_hash_str, truncate=16):016x}"
        return str(id(data))
    ...
    def _normalize_dict_list_to_triples(
        self, data: list[Any]
        ...
        for key, expected_value in criteria.items():
            if key == "*":
                ...
                for subject, predicate in self.by_value[expected_value]:
                    if key in (predicate, "*"):
                        matching_triples.append((subject, predicate, expected_value))
