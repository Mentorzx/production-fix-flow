...
try:
    ...
    KGPreprocessingPipeline = None  # type: ignore
...
class EntityRelationIndexer:
    ...
    def index_triples(
        ...
        indexed_np = indexed_dataframe.cast(
            {c: pl.UInt32 for c in indexed_dataframe.columns}
        ).to_numpy(order="c")
...
class KGPreprocessor(DataPreprocessorInterface):
    ...
    def _run_centralized_preprocessing(self) -> bool:
        ...
        try:
            ...
            self.file_manager.save(result.stats, stats_path)
    ...
    def _load_raw_parquet_splits(self) -> dict[str, pl.DataFrame]:
        ...
        for split_name in ["train", "valid", "test"]:
            ...
            if self.file_manager.exists(split_path):
                logger.debug(f"split_loading name={split_name} path={split_path}")
                payload = self.file_manager.read(split_path, lazy=True, streaming=True)
    ...
    def _filter_orphan_entities(
        ...
        for split_name in ["valid", "test"]:
            ...
            filtered_df = original_df.filter(
                pl.col("s").is_in(train_ents_list) & pl.col("o").is_in(train_ents_list)
            )
        ...
        train_test_overlap = int(train_entities.is_in(test_entities.implode()).sum())
    ...
    def _homogenize_and_map(
        ...
        all_relations = pl.concat(
            [df["p"] for df in normalized_splits.values()]
        ).unique()
    ...
    def update_maps_and_reindex_from_rules(self) -> None:
        ...
        relation_bundle = self.file_manager.read(
        ...
        for split in ["train", "valid", "test"]:
            ...
            if not self.file_manager.exists(split_path):
                continue
            split_bundle = self.file_manager.read(split_path, streaming=True)
