...
class MultiObjectiveOptimizer:
    ...
    def extract_pareto_front(
        self,
        study: optuna.Study,
    ) -> Tuple[List[optuna.trial.FrozenTrial], pd.DataFrame]:
        """
...
class AdvancedHyperparameterOptimizer:
    """
    Unified advanced hyperparameter optimizer with all 6 features.
    ...
    def __init__(
        ...
        self.multi_obj = MultiObjectiveOptimizer() if enable_multi_objective else None
        self.nas = NeuralArchitectureSearch() if enable_nas else None
        self.distributed = DistributedOptimizer() if enable_distributed else None
        self.reporting = OptunaReporting() if enable_dashboard else None
        self.importance = ImportanceAnalyzer() if enable_importance else None
        self.transfer = TransferLearningOptimizer() if enable_transfer else None
        
        logger.info("Advanced optimizer initialized with:")
        if self.multi_obj:
            logger.info("  ✅ Multi-objective optimization")
        if self.nas:
            logger.info("  ✅ Neural Architecture Search")
        if self.distributed:
            logger.info("  ✅ Distributed optimization (Ray Tune)")
        if self.reporting:
            logger.info("  ✅ Automated reporting (Dashboard)")
        if self.importance:
            logger.info("  ✅ Importance analysis")
        if self.transfer:
    ...
    def optimize(
        ...
        """
        Run advanced optimization with all enabled features.
        
        Args:
            objective_fn: Objective function to optimize
            search_space: Hyperparameter search space
            n_trials: Number of trials
            use_transfer: Use transfer learning if available
        
        Returns:
            Optuna study with results
        """
        # Get warm-start params if transfer learning enabled
        warmstart_params = []
        if self.transfer and use_transfer:
            warmstart_params = self.transfer.get_warmstart_params(search_space)
        
        # Create sampler
        sampler = (
            self.transfer.create_warmstart_sampler(warmstart_params)
            if warmstart_params
        ...
        # Create study
        if self.multi_obj:
            study = self.multi_obj.create_study()
        else:
            study = optuna.create_study(
                direction='maximize',
                sampler=sampler,
        ...
        # Save to transfer learning history
        if self.transfer:
            self.transfer.save_history(study)
        
        # Generate reports
        if self.reporting:
            self.reporting.generate_report(study)
        
        # Analyze importance
        if self.importance:
            self.importance.save_importance_analysis(study)
        
        # Save Pareto front if multi-objective
        if self.multi_obj:
            self.multi_obj.save_pareto_front(study)
...
if __name__ == "__main__":
    ...
    print("  1. Multi-objective optimization (Pareto front)")
    print("  2. Neural Architecture Search (NAS)")
    print("  3. Distributed optimization (Ray Tune)")
    print("  4. Automated reporting (Optuna Dashboard)")
    print("  5. Hyperparameter importance analysis")
    print("  6. Transfer learning from previous runs")
