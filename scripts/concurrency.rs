    ...
    if cfg.use_uring {
        #[cfg(feature = "linux_uring")]
        {
            monoio::start::<monoio::IoUringDriver, _>(async move {
                ...
                // Create one DCtx per worker and reuse for all frames (ZSTD best practice).
                let mut zstd_dctx = zstd_safe::DCtx::create();
                loop {
                        ...
                        Ok((buf, bytes)) => {
                            let payload = match req.mode {
                                ReadMode::Zstd {
                                    ...
                                    match decompress_zstd_with_ctx(&mut zstd_dctx, buf.as_slice()) {
                                        Ok(out) => {
                                            pool.release(buf);
...
fn decompress_zstd_with_ctx(
    dctx: &mut zstd_safe::DCtx<'static>,
    input: &[u8],
) -> Result<Vec<u8>> {
    // Reuse the context's internal allocations/tables across frames, but reset "session" state.
    dctx.reset(zstd_safe::ResetDirective::SessionOnly).map_err(|e| {
        ConcurrencyError::Io(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("zstd reset: {e}"),
        ))
    ...
    let Some(len_u64) = expected else {
        ...
        let mut decoder = zstd::stream::read::Decoder::new(input).map_err(ConcurrencyError::Io)?;
    ...
    let len: usize = len_u64.try_into().map_err(|_| {
        ConcurrencyError::Io(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            "zstd frame too large",
        ))
    })?;

    let mut out = vec![0u8; len];
    let written = dctx.decompress(&mut out, input).map_err(|code| {
        ConcurrencyError::Io(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
...
/// Backwards-compatible helper: allocates a fresh context and decompresses one frame.
/// Prefer `decompress_zstd_with_ctx()` on hot paths.
fn decompress_zstd(input: &[u8]) -> Result<Vec<u8>> {
    let mut dctx = zstd_safe::DCtx::create();
    decompress_zstd_with_ctx(&mut dctx, input)
}
