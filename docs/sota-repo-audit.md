...
7. **Reivindicação SOTA do Advisor:** universalmente não é defensável, mas agora há uma reivindicação local defensável: `advisor_domain_edge_gp` superou GP-BO no cenário sintético `edge_capacity_expandable` sob 30 trials, 80 seeds, Advisor isolado e regra Holm pareada (média 0.646533 vs 0.636241; delta +0.010292; IC95 [0.002510, 0.019958]; 6 vitórias, 74 empates, 0 derrotas; Wilcoxon/Holm p=0.013854). A afirmação aceitável é restrita: SOTA neste protocolo sintético expansível, não superioridade universal.

## Por que a reivindicação universal ainda não sustenta

O resultado atual sustenta uma afirmação local: neste benchmark sintético, o Advisor supera GP-BO quando há espaço inicial subdimensionado, domínio superior explícito e gate de validação para expansão de `embedding_dim`. Ele não sustenta uma afirmação universal por quatro razões:
...
Hipótese expansível confirmada localmente: foi adicionado o cenário `edge_capacity_expandable`, em que o espaço inicial mantém `embedding_dim<=512`, mas o domínio permitido vai até 1024 e o ótimo sintético fica acima do teto inicial. A política `advisor_domain_edge_gp` força expansão de domínio apenas quando a elite encosta na borda superior de `embedding_dim` e a validação Wilson-LB do Advisor passa de 0.45. Em 80 seeds, 30 trials e Advisor isolado, o resultado foi positivo, sem perdas e significativo contra GP-BO (média 0.646533 vs 0.636241; delta +0.010292; IC95 [0.002510, 0.019958]; 6 vitórias, 74 empates, 0 derrotas; Wilcoxon/Holm p=0.013854). Esta é uma reivindicação local defensável: SOTA no protocolo sintético PFF-KGC `edge_capacity_expandable` sob orçamento de 30 trials.
...
|          8 | Backtest do Advisor ainda usa recorte curto de 25 trials         | Alto    | Alto     | Médio | Média    | Corrigido como evidência 50-complete: estudo `deep_research_advisor_real50_gpu_20260506` auditado com 50 trials completos no dashboard, melhor objetivo 0.469644, 21 recomendações, 37 prefixos avaliados, hit-rate direcional 0.7903 e validação Wilson-LB 0.7733. O benchmark pareado sintético agora cobre múltiplos cenários, Random, TPE, TPE+Hyperband, GP-BO, Advisor e ablations, com IC bootstrap, vitórias pareadas, Wilcoxon, Friedman e Holm. Resultado honesto: as melhorias curtas iniciais contra GP-BO não sobreviveram a mais seeds até a correção de mudança material; depois, `advisor_domain_edge_gp` sustentou uma reivindicação SOTA local somente no protocolo sintético `edge_capacity_expandable` sob 30 trials e 80 seeds. PDF ABNT não deve reivindicar superioridade universal nem extrapolar para datasets reais. |
