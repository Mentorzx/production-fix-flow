...
1. **Efeito ainda pequeno contra GP-BO:** no protocolo multi-cenário com 20 seeds, 30 trials e Advisor isolado, o sinal inicial de `advisor_embedding_upper_gp` foi pequeno. Ao auditar os pares não empatados, descobriu-se que `embedding_dim` já estava no teto (`high=512`), então o patch não alterava o espaço e apenas reiniciava o GP-BO. O benchmark agora detecta ausência de mudança material, mantém o sampler e registra `static_restart_skipped=no_material_change`; no diagnóstico com seeds 67, 89 e 251, os três pares viraram empate exato contra GP-BO. Isso elimina um falso positivo metodológico em vez de criar uma reivindicação SOTA.
