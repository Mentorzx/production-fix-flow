...
7. **Reivindicação SOTA do Advisor:** ainda não é defensável como universal nem como SOTA local robusto. O benchmark pareado foi ampliado para múltiplos cenários sintéticos PFF-KGC, com IC bootstrap, Wilcoxon pareado e Friedman. A melhor triagem curta com 3 seeds (`advisor_edge_gated_gp`) superou levemente GP-BO, mas esse sinal caiu em 20 seeds. A ablation mais conservadora (`advisor_embedding_upper_gp`) ficou positiva e sem derrotas em 20 seeds, mas o recorte `edge_capacity` com 50 seeds ainda não foi significativo. O No Free Lunch impede transformar esses sinais locais em superioridade geral.

## Por que a reivindicação universal ainda não sustenta

O resultado atual sustenta uma afirmação local fraca: neste benchmark sintético, políticas Advisor podem igualar GP-BO na maior parte dos pares e obter ganhos quando a evidência aponta expansão superior de `embedding_dim`, mas os ganhos ainda são pequenos e estatisticamente frágeis. Ele não sustenta uma afirmação universal por quatro razões:

1. **Efeito ainda pequeno contra GP-BO:** no protocolo multi-cenário com 20 seeds, 30 trials e Advisor isolado, GP-BO teve média 0.750145 e `advisor_embedding_upper_gp` teve média 0.751337; delta médio vs GP-BO +0.001192; IC95 bootstrap [0.000000, 0.003575]; 2 vitórias, 78 empates e 0 derrotas; Wilcoxon unilateral p=0.089856. No recorte `edge_capacity` com 50 seeds, o delta ficou +0.001747, IC95 [-0.000320, 0.005528], 2 vitórias, 47 empates, 1 derrota e p=0.296490. Isso é promissor como heurística de baixo dano, mas não estatisticamente forte.
...
Recorte positivo defensável: a política `advisor_embedding_upper_gp` só aplica a recomendação quando a borda superior influente é exclusivamente compatível com `embedding_dim` e o patch também é restrito a `embedding_dim`; nos demais casos, mantém GP-BO e não consome trial adicional. A hipótese científica é que, sob orçamento curto, o Advisor é útil como camada de baixo dano para detectar capacidade subdimensionada, não como substituto geral de GP-BO.
