...
7. **Reivindicação SOTA do Advisor:** ainda não é defensável como universal. O benchmark pareado foi ampliado para múltiplos cenários sintéticos PFF-KGC, com IC bootstrap, Wilcoxon pareado e Friedman. A melhor triagem curta agora é `advisor_edge_gated_gp`, que superou levemente GP-BO no agregado, mas com efeito pequeno, IC tocando zero e p-valor não significativo. O No Free Lunch impede transformar uma vitória local em superioridade geral sobre todas as classes.

## Por que a reivindicação universal ainda não sustenta

O resultado atual sustenta uma afirmação local: neste benchmark sintético, políticas Advisor melhoram TPE puro em média, e a melhor variante de triagem (`advisor_edge_gated_gp`) ficou acima de GP-BO no agregado curto sem derrotas pareadas. Ele não sustenta uma afirmação universal por quatro razões:

1. **Efeito ainda pequeno contra GP-BO:** no protocolo curto multi-cenário executado localmente (`smooth_kgc`, `narrow_ridge`, `conditional_regularized`, `edge_capacity`; 3 seeds; 30 trials; `--no-isolated-advisor`), GP-BO teve média 0.783902 e `advisor_edge_gated_gp` teve média 0.786175; delta médio vs GP-BO +0.002273; IC95 bootstrap [0.000000, 0.006819]; 1 vitória e 0 derrotas pareadas; Wilcoxon unilateral para Advisor > GP-BO p=0.5. Isso é promissor, mas não estatisticamente forte.
...
Recorte positivo defensável: a política `advisor_edge_gated_gp` só permite reinício estático quando há evidência de parâmetros influentes próximos à borda e pelo menos uma recomendação `expand_upper`; nos demais casos, mantém o GP-BO e não consome trial adicional. Na triagem curta, ela aplicou patch apenas em `edge_capacity`/seed 17 (`embedding_dim`, `temperature`) e ganhou +0.027276 naquele par, empatando os demais pares contra GP-BO. Isso é uma hipótese promissora sobre espaços onde o ótimo está além da borda superior observada, não uma reivindicação SOTA.
...
Baseline multi-fidelidade: `tpe_hyperband` foi adicionado com intermediários simulados e `HyperbandPruner` real do Optuna. Na triagem curta mais recente, perdeu para GP-BO em todos os pares (média 0.659112; delta -0.124790; IC95 [-0.164444, -0.093225]). Isso confirma que o pruner é útil como baseline obrigatório, mas não resolveu o gargalo central deste protocolo.
