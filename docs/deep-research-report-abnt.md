...
Uma triagem posterior, executada com Advisor isolado em processo separado, 30 trials, sementes [11, 17, 23] e quatro cenários sintéticos (`smooth_kgc`, `narrow_ridge`, `conditional_regularized`, `edge_capacity`), adicionou baselines Random, TPE+Hyperband, GP-BO, Advisor completo e ablations. Nessa triagem, uma política conservadora `advisor_edge_gated_gp` manteve o comportamento do GP-BO quando a evidência de borda era fraca e só reiniciou o espaço quando havia parâmetros influentes próximos à borda superior. O resultado agregado foi `advisor_edge_gated_gp` média 0.786175 contra GP-BO média 0.783902, delta +0.002273, IC95 bootstrap [0.000000, 0.006819], 1 vitória e 0 derrotas pareadas, Wilcoxon unilateral p=0.5. A leitura correta é que o gate reduz dano em cenários onde o Advisor não tem sinal suficiente e cria uma hipótese promissora para espaços com ótimo além da borda observada; ainda não há significância estatística para reivindicação SOTA.

Ao ampliar a amostra para 20 seeds, o sinal de `advisor_edge_gated_gp` não se sustentou: o delta médio contra GP-BO caiu para -0.004323, com 7 vitórias, 66 empates, 7 derrotas e p=0.801637. A análise dos erros indicou que expansões conjuntas de `temperature`, `learning_rate` e outros parâmetros causavam perdas grandes. Por isso, testou-se uma ablation mais conservadora, `advisor_embedding_upper_gp`, que só aplica patch quando a borda superior influente e os parâmetros alterados são restritos a `embedding_dim`. Nessa configuração, em quatro cenários e 20 seeds, o resultado foi média 0.751337 contra 0.750145 do GP-BO, delta +0.001192, IC95 [0.000000, 0.003575], 2 vitórias, 78 empates, 0 derrotas e p=0.089856. No recorte `edge_capacity` com 50 seeds, o delta foi +0.001747, IC95 [-0.000320, 0.005528], 2 vitórias, 47 empates, 1 derrota e p=0.296490. Assim, a conclusão atual é que o Advisor pode atuar como gate conservador de baixo dano para detectar capacidade subdimensionada, mas ainda não sustenta superioridade estatística SOTA contra GP-BO.
...
Em seguida, testou-se um cenário sintético explicitamente expansível, `edge_capacity_expandable`: o espaço inicial preserva `embedding_dim<=512`, mas o domínio permitido chega a 1024 e o ótimo sintético está acima do teto inicial. A política `advisor_domain_edge_gp` força expansão para o domínio apenas quando a elite encosta na borda superior de `embedding_dim` e a validação Wilson-LB do Advisor supera 0.45. Em 80 seeds, 30 trials e Advisor isolado, o resultado foi média 0.646533 contra 0.636241 do GP-BO, delta +0.010292, IC95 [0.002510, 0.019958], 6 vitórias, 74 empates, 0 derrotas e Wilcoxon/Holm p=0.013854. Portanto, há uma reivindicação local defensável: o Advisor é SOTA neste protocolo sintético expansível sob orçamento de 30 trials. Essa frase não deve ser extrapolada para superioridade universal nem para datasets reais ainda não avaliados.
