...
const computeSummary = (trials) => {
    ...
    const ssRes = completed.reduce((a, b) => {
        const pred = slope * b.x + intercept;
            ...
            <div className="flex flex-col h-full gap-2 min-h-0">
                ...
                <div className="grid grid-cols-2 gap-2 flex-shrink-0">
                    <div className="p-2 rounded-lg border border-zinc-800 bg-zinc-950/40 flex flex-col justify-center">
                        <div className="text-[9px] uppercase tracking-widest text-zinc-500 mb-0.5">R² Check</div>
                        <div className="text-sm font-bold text-orange-400 font-mono tracking-tight">{summary.r2.toFixed(3)}</div>
                    </div>
                    <div className="p-2 rounded-lg border border-zinc-800 bg-zinc-950/40 flex flex-col justify-center">
                        <div className="text-[9px] uppercase tracking-widest text-zinc-500 mb-0.5">Slope</div>
                        <div className={`text-sm font-bold font-mono tracking-tight ${summary.slope >= 0 ? "text-lime-400" : "text-rose-400"}`}>
                            {summary.slope >= 0 ? "+" : ""}{summary.slope.toFixed(4)}
                        </div>
                    </div>
                    <div className="p-2 rounded-lg border border-zinc-800 bg-zinc-950/40 flex flex-col justify-center">
                        <div className="text-[9px] uppercase tracking-widest text-zinc-500 mb-0.5">RMSE</div>
                        <div className="text-sm font-bold text-zinc-300 font-mono tracking-tight">{summary.rmse.toFixed(4)}</div>
                    </div>
                    <div className="p-2 rounded-lg border border-zinc-800 bg-zinc-950/40 flex flex-col justify-center">
                        <div className="text-[9px] uppercase tracking-widest text-zinc-500 mb-0.5">Corr</div>
                        <div className="text-sm font-bold text-zinc-300 font-mono tracking-tight">{summary.correlation.toFixed(3)}</div>
                    </div>
                </div>

                {/* Projections Table */}
                <div className="p-2 rounded-lg border border-zinc-800 bg-zinc-950/40 flex-1 flex flex-col min-h-0">
                    <div className="text-[9px] uppercase tracking-widest text-zinc-500 mb-2">Projeções Futuras</div>
                        ...
                        <table className="w-full text-[10px] text-zinc-300">
                            ...
                            <tbody>
                                {summary.projections.map((p) => (
                                    <tr key={p.label} className="border-b border-zinc-800/30 last:border-0 hover:bg-white/5">
