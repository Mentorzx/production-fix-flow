...
const pickAxisKeys = (numericKeys) => {
    ...
    const yKey = pick(["embedding", "embed", "hidden"]) || numericKeys.find((k) => k !== xKey) || null;
    return { xKey, yKey };
};
...
export const ContourPlotCard = ({ trials }) => {
    const contour = useMemo(() => {
        ...
        const xBins = 16;
        const yBins = 12;
        const grid = Array.from({ length: yBins }, () => Array.from({ length: xBins }, () => ({ sum: 0, count: 0 })));
    ...
    return (
        <Card title="Contour Plot" className="h-full" icon={Layers} helpText={helpText}>
            <ChartFrame className="flex flex-col h-full gap-3">
                <WithData when={hasData} empty="Dados insuficientes para superfície 3D" emptyClassName="text-zinc-500">
                    ...
                    <div className="flex-1 min-h-0 grid gap-3" style={{ gridTemplateRows: "1fr auto" }}>
                        <div className="relative h-full min-h-[220px] border border-zinc-800 rounded-lg bg-zinc-950/40 p-2">
                            ...
                            >
                                {contour.grid.flat().map((cell) => {
                                    const t = cell.value == null || contour.zMax === contour.zMin
                                        ? 0
                        ...
                        <div className="flex items-center justify-between text-[10px] text-zinc-500 font-mono">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="h-1 w-24 rounded-full" style={{ background: `linear-gradient(90deg, ${colors.orange}, ${colors.lime})` }}></span>
