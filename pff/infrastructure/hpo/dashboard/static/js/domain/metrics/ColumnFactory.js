...
export const efficiencyColumn = () => ({
    ...
    render: (_v, row) => {
        ...
        return eff
            ? <span className="font-mono" style={{ color: 'var(--viz-palette-4-yellow)' }}>{(eff * 100).toFixed(2)}%</span>
            : '—';
    }
});
...
export const rankingColumns = () => RANKING_KEYS.map(key => heatmapColumn(key, { group: 'ranking', width: 'auto' }));
