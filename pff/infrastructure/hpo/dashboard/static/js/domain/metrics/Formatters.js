...
export const resolveMetricValue = (row, key) => {
    ...
    const mapping = {
        ...
        'best_mcc': ['best_mcc', 'best_val_mcc', 'metrics.best_mcc', 'metrics.best_val_mcc', 'learner.best_mcc', 'learner.best_val_mcc'],
        'best_mrr': ['best_mrr', 'best_val_mrr', 'metrics.best_mrr', 'metrics.best_val_mrr', 'kge.best_mrr', 'best_val_mrr'],
        ...
        'loss': ['loss', 'metrics.loss', 'val_loss', 'metrics.val_loss', 'train_loss', 'metrics.train_loss', 'metrics.binary_loss', 'binary_loss']
    };
    ...
    if (candidates) {
        for (const path of candidates) {
            ...
            if (val !== undefined && val !== null) {
                ...
                if (key !== 'duration' && state && state !== 'COMPLETE' && typeof val === 'number' && val === 0) {
                    return null;
                }
                return val;
            }
    ...
    if (row[key] !== undefined && row[key] !== null) {
        ...
        if (key !== 'duration' && state && state !== 'COMPLETE' && typeof row[key] === 'number' && row[key] === 0) {
            return null;
        }
        return row[key];
    }
