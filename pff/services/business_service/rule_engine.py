...
from multiple sources (manual JSON, AnyBURL TSV).
...
Performance:
    - Uses DiskCache for AnyBURL rules to avoid re-parsing TSV files
    - Rule aggregation to deduplicate identical patterns
"""
...
class RuleEngine:
    ...
    def _load_and_aggregate_rules(self, filepath: Path) -> list[Rule]:
        ...
        Uses Polars lazy API for optimized query execution on large TSV files.
        ...
        rules = []
        try:
            # Use lazy API for streaming and query optimization on large files
            lf = pl.scan_csv(
                filepath,
                separator="\t",
                has_header=False,
                new_columns=["total_predictions", "correct_predictions", "confidence", "rule_text"],
            )

            # Collect with streaming for memory efficiency on large files
            df = lf.collect(engine="streaming")

            # Process rules using columnar access (more efficient than to_dicts)
            total_preds = df["total_predictions"].to_list()
            correct_preds = df["correct_predictions"].to_list()
            confidences = df["confidence"].to_list()
            rule_texts = df["rule_text"].to_list()

            for idx, (total, correct, conf, rule_text) in enumerate(
                zip(total_preds, correct_preds, confidences, rule_texts)
            ):
                try:
                    ...
                    logger.debug(f"Error processing AnyBURL rule #{idx}: {e}")
                    continue

            logger.success(f" {len(rules)} regras AnyBURL carregadas de {filepath}")
            logger.info(f" Agregando {len(rules)} regras...")
            aggregated_rules = aggregate_duplicate_rules(rules)
            logger.success(f" {len(aggregated_rules)} regras unicas apos agregacao")
            ...
            logger.error(f"Failed to load AnyBURL rules: {e}")
            raise
    ...
    def load_anyburl_rules(self, filepath: Path | None = None) -> None:
        ...
        **v10.8.0 - DiskCache Integration:**
        ...
        cache_ttl = cache_cfg.get("rules_ttl_days", 7) * 24 * 3600
        cache = DiskCache(root=settings.CACHE_DIR / "aggregated_rules")
