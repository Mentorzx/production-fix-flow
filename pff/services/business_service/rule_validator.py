...
Performance:
    ...
    - Supports Numba-accelerated triple matching
...
class IndexedViolationStrategy:
    ...
    def find_violations(
        ...
        find_rule_violations_indexed(
            rule.body, triples, triple_index, 0, {}, violations, rule,
            max_depth=max_depth
        )
        return violations
...
class StandaloneViolationStrategy:
    ...
    def __init__(self, encoder: VocabularyEncoder | None = None):
        """Initialize with optional Numba encoder."""
...
class ViolationStrategyFactory:
    ...
    @staticmethod
    def create(use_index: bool = True, encoder: VocabularyEncoder | None = None) -> ViolationFindingStrategy:
        """
        Create a violation finding strategy.

        Args:
            use_index: Whether to use indexed strategy (default: True)
            encoder: Optional Numba encoder for standalone strategy
...
class RuleValidator:
    ...
    def validate_rules(
        ...
        self._emit_event(ValidationEvent(
...
def find_rule_violations_standalone(
    ...
    **DEPRECATED:** Use find_rule_violations_indexed for O(1) triple lookups.
    ...
    Args:
        encoder: Optional VocabularyEncoder for Numba acceleration
    """
    if pred_idx >= len(body_predicates):
