...
- Adapter Pattern: accepts numpy/polars/pandas inputs and returns SHAP-native explanations.
"""
...
from pff.config import EXPLAINABILITY_CONFIG_PATH
...
class ShapExplainerService:
    ...
    def _load_config(
        ...
        if resolved_path.exists():
            try:
                content = self.file_manager.read(resolved_path)
    ...
    def _build_explainer(self, model: Any, background: np.ndarray):
        ...
        except Exception as exc:  # pragma: no cover - dependency failure should be explicit
        ...
        try:
            ...
            logger.warning(f"SHAP explainer creation failed; falling back to KernelExplainer: {exc}")
            try:
    ...
    def _sample_rows(self, data: np.ndarray, max_rows: int) -> np.ndarray:
        if data.shape[0] <= max_rows:
            return data
        indices = np.random.choice(data.shape[0], size=max_rows, replace=False)
        return data[indices]
