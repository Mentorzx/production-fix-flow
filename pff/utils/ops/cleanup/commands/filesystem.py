...
class NestedDirCleanCommand(CleanupCommand):
    ...
    def execute(self) -> None:
        ...
        Finds all directories named `dirname` under ROOT_DIR and removes them.
        """
        for d in settings.ROOT_DIR.rglob(self.dirname):
            if d.is_dir():
                FileOps.rmtree_sync(d, ignore_errors=True)
...
class PyCacheCleanCommand(CleanupCommand):
    ...
    def execute(self) -> None:
        ...
        Removes all `__pycache__` directories under ROOT_DIR.
        """
...
class TrainingArtifactsCleanCommand(CleanupCommand):
    ...
    def execute(self) -> None:
        ...
        """
        artifacts_patterns = [
            settings.OUTPUTS_DIR / "rotate" / "temp_*",
            settings.OUTPUTS_DIR / "rotate" / "*_temp.yaml",
