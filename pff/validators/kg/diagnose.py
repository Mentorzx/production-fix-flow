...
def diagnosticar_entity_map(path: Path):
    print("1⃣  Verificando entity_map em", path)
    if not path.exists():
        print("     entity_map.parquet não encontrado, pulei esta etapa.\n")
        return
    df = fm.read(path)
...
def diagnosticar_npy(split: str, base: Path):
    ...
    print(f"2⃣  Verificando {split}.npy em {path}")
    if not path.exists():
        print("     Arquivo não encontrado, pulei esta etapa.\n")
        return
    arr = np.load(path)
...
def diagnosticar_regras(path: Path):
    print("3⃣  Verificando rules_anyburl.tsv em", path)
    if not path.exists():
        print("     Arquivo não encontrado, pulei esta etapa.\n")
        return
    from pff.utils import FileManager
...
def check_anyburl_rules(path: Path):
    print("4⃣  Checando literal específico em rules_anyburl.tsv", path)
    if not path.exists():
...
def diagnosticar_entidades_orfas(base: Path):
    print("5⃣  Verificação de Entidades Órfãs")
    mp = base / "entity_map.parquet"
    print("    Carregando mapeamento de entidades:", mp)
    if not mp.exists():
        print("     entity_map.parquet não encontrado.\n")
        return
    em = fm.read(mp)
