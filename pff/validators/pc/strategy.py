...
or compilation failures). It is a drop-in replacement for the symbolic
aggregation interface used by the Hierarchical Ensemble.
"""
...
from pff.validators.pc.compiler import (
    CircuitCompilationError,
    RuleToCircuitCompiler,
)
from pff.validators.pc.inference import PCInferenceEngine
from pff.utils.logger import logger


class ProbabilisticCircuitStrategy(SymbolicAggregatorStrategy):
    ...
    def __init__(
        ...
        self._fallback_strategy = NoisyOrStrategy(base_confidence=base_confidence)

    @property
    def name(self) -> str:
        return AggregationStrategy.PC.value
    ...
    def aggregate(
        ...
        return self._fallback_strategy.aggregate(confidences, weights)
